...
public class NavObject{
    public GameObject navLabel;
    public Vector2d coord;

    public NavObject(GameObject n, Vector2d c){
        navLabel = n;
        coord = c;
    }
}

public class navCreatorGround : MonoBehaviour
{
    ...
    bool firstTImeGetUserLocation = true;
    public static Vector2d userPreviousLocation;
    ...
    public void setUserLocationNav(string str)
    {
        ...
        var tempLocation = userCurrentLocation - userPreviousLocation;
        ...
        if (firstDraw){
            ...
            initHeading = headingControl.headingInfo.heading;
        }
        ...
        if (distTravel >= 5){
            cleanNavObjectList();
            Debug.Log("Recalibrated with Heading: " + headingControl.heading);
    ...
    public void setUserCurrentLocation(double lat, double lng)
    {
        ...
        if(firstTImeGetUserLocation == true){
            userCurrentLocation = Conversions.GeoToWorldPosition(lat, lng, new Vector2d(0f, 0f));
            userPreviousLocation = userCurrentLocation;
            firstTImeGetUserLocation = false;
        }else{
            userPreviousLocation = userCurrentLocation;
            userCurrentLocation = Conversions.GeoToWorldPosition(lat, lng, new Vector2d(0f, 0f));
    ...
    public void drawPoints(){
        ...
        if (info.coordinates != null)
        {
            ...
            foreach (var routePoint in info.coordinates)
            {
                ...
                {
                    navObject = Instantiate(destinationMarker) as GameObject;
                }
                else
                {
                    navObject = Instantiate(directionsArrow) as GameObject;
                ...
                navObjectLocation = toolsLib.getRotatedLocation(navObjectLocation, new Vector3(0, 0, 0), Vector3.up, 0);
            ...
            groundPlane.transform.position = toolsLib.getRotatedLocation(groundPlane.transform.position, Vector3.zero, Vector3.up, headingControl.headingInfo.isValid ? -headingControl.headingInfo.heading : 0);
            if (!firstDraw)
            {
                groundPlane.transform.parent = GameObject.FindWithTag("Route").transform;
            }
    ...
    public void reCalibratePoints(){
        ...
        //euler.y = initHeading - headingControl.heading;

        route.transform.rotation = Quaternion.Euler(0f, 0f, 0f);

        //For clean the debug info
        //Debug.Log("Route rotation (zero)" + route.transform.rotation);

        route.transform.Rotate(Vector3.up, -headingControl.headingInfo.heading);
        //For clean the debug info
        //Debug.Log("Route rotation (recal)" + route.transform.rotation);

        route.transform.Rotate(1f, 1f, 1f);
        //StartCoroutine(RotateLerp(route, Vec, .3f));


    }

    IEnumerator RotateLerp(GameObject gameObject, Quaternion end, float time)
    {
        ...
        while (i < 1.0f)
        {
            ...
            gameObject.transform.localRotation = Quaternion.Lerp(gameObject.transform.localRotation, end, i);
            yield return null;
        }
    ...
    public void checkDist(NavObject navObject, double dist)
    {
        ...
        //For clean the debug info
        //Debug.Log("Dist Obj diff: " + (dist-distObj));
